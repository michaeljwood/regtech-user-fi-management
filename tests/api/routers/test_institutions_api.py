...
class TestInstitutionsApi:
    def test_get_institutions_unauthed(
    ...
    def test_get_institutions_authed(
        ...
        get_institutions_mock = mocker.patch(
            "entities.repos.institutions_repo.get_institutions"
        )
    ...
    def test_create_institution_unauthed(
        self, app_fixture: FastAPI, unauthed_user_mock: Mock
    ):
        client = TestClient(app_fixture)
        res = client.post(
            "/v1/institutions/", json={"name": "testName", "lei": "testLei"}
        )
        assert res.status_code == 403

    def test_create_institution_authed(
        self, mocker: MockerFixture, app_fixture: FastAPI, authed_user_mock: Mock
    ):
        upsert_institution_mock = mocker.patch(
            "entities.repos.institutions_repo.upsert_institution"
        )
    ...
    def test_create_institution_authed_no_permission(
    ...
    def test_get_institution_unauthed(
        self, app_fixture: FastAPI, unauthed_user_mock: Mock
    ):
        client = TestClient(app_fixture)
    ...
    def test_add_domains_authed_no_permission(
        self, app_fixture: FastAPI, auth_mock: Mock
    ):
        claims = {
            "name": "test",
