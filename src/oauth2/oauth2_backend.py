import logging
from typing import Coroutine, Any, Dict, List, Tuple, Optional
from fastapi import HTTPException
from pydantic import BaseModel
from starlette.authentication import AuthCredentials, AuthenticationBackend, BaseUser, UnauthenticatedUser, SimpleUser
from fastapi.security import OAuth2AuthorizationCodeBearer
from starlette.requests import HTTPConnection

from . import oauth2_admin
...
class AuthenticatedUser(BaseUser, BaseModel):
    ...
    def from_claim(cls, claims: Dict[str, Any]) -> "AuthenticatedUser":
        return cls(claims=claims, name=claims.get("name"), username=claims.get("preferred_username"), email=claims.get("email"), id=claims.get("sub"))
    ...
    async def authenticate(self, conn: HTTPConnection) -> Coroutine[Any, Any, Tuple[AuthCredentials, BaseUser] | None]:
        try:
            ...
            if claims is not None:
                auths = self.extract_nested(claims, "resource_access", "realm-management", "roles") + \
                self.extract_nested(claims, "resource_access", "account", "roles") + ["authenticated"]
